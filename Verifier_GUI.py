...
class App():
    def __init__(self):
        self.filepath = ''
        self.data = ''
        self.app = gui("Microsoft ElectionGuard Verifier", "800x600")

        self.app.addButton("Upload", self.press)
        self.app.addLabel("Filepath", "")
        self.app.addButton("Verify", self.verify)
        self.app.addLabel("Status", "")
        self.app.addMeter("progress")

        self.app.go()

    def press(self):
        self.filepath = self.app.openBox(title=None, dirName=None, fileTypes=None, asFile=False, parent=None, multiple=False,
                               mode='r')
        self.app.setLabel("Filepath", self.filepath)

    def function_modular_product(self, contest_index, selection_index, p):
        A, B = 1, 1
    ...
    def verify(self):
        ...
        joint_public_key_K = int(self.data['joint_public_key'])
        ...
        for trustee_key_list in self.data['trustee_public_keys']:
            for trustee_key_info in trustee_key_list:
                ...
                break
        i = 1
        prog_val = (i*100)/len(self.data['trustee_public_keys'])

        self.app.setLabel("Status", 'Computing Public Key Validity')

        self.app.setMeter("progress", prog_val)
        ...
        for trustee_key_list in self.data['trustee_public_keys']:
            ...
            for trustee_key_info in trustee_key_list:
                ...
                rhs = pow(
                    pow(int(trustee_key_info['proof']['commitment']), 1, p) * pow(int(trustee_key_info['public_key']),
        ...
        i = 1
        prog_val = (i * 100) / len(self.data['cast_ballots'])

        self.app.setLabel("Status", 'Computing Cast Ballot Validity')
        self.app.setMeter("progress", prog_val)
        for cast_ballot in self.data['cast_ballots']:
            print('Computing ', i, 'of', len(self.data['cast_ballots']), ' ballot')
            for contest in cast_ballot["contests"]:
                ...
                for selection in contest['selections']:
                    ...
                    if not (int(selection['message']['public_key']) < p):
                        print('Public Key (Alpha): ', int(selection['message']['public_key']),
                    ...
                    if not (int(selection['message']['ciphertext']) < p):
                        print('Ciphertext(Beta): ', int(selection['message']['ciphertext']),
                              ' not a part of subgroup p')
                        ballot_status_flag = False

                    if not (pow(int(selection['message']['ciphertext']), q, p) == 1):
                        print('Ciphertext(Beta): ', int(selection['message']['public_key']),
                              ' not a part of subgroup Z_p*')
                        ballot_status_flag = False
                    ...
                    if not (int(selection['zero_proof']['challenge']) < q):
                        print('Challenge (c_0): ', int(selection['zero_proof']['challenge']),
                              ' not a part of subgroup Z_q')
                        ballot_status_flag = False
                    ...
                    lhs1 = pow(g, int(selection['zero_proof']['response']), p)
                    rhs1 = pow(
                        pow(int(selection['zero_proof']['commitment']['public_key']), 1, p) * pow(
                            int(joint_public_key_K), int(
                    ...
                    lhs2 = pow(g, int(selection['one_proof']['response']), p)
                    rhs2 = pow(pow(int(selection['one_proof']['commitment']['public_key']), 1, p) * pow(
                        int(joint_public_key_K),
                        int(selection[
                                'one_proof'][
                                'challenge']),
                        p), 1, p)
                    ...
                    lhs4 = pow(
                        pow(g, int(selection['one_proof']['challenge']), p) * pow(
                            int(selection['message']['public_key']),
                ...
                lhs = pow(
                    pow(g, int(contest["max_selections"]) * int(contest['num_selections_proof']['challenge']), p) * pow(
                        A, int(contest["num_selections_proof"]["response"]), p), 1, p)
        ...
        j = 1
        prog_val = (j*100)/len(self.data['contest_tallies'])
        self.app.setLabel("Status", "Computing Ballot Tally")
        self.app.setMeter("progress", prog_val)


        for index, contest_tally in enumerate(self.data['contest_tallies']):
            print('Computing contest tally', j, 'of ', len(self.data['contest_tallies']))
            for index1, contest_tally_value in enumerate(contest_tally):
                ...
                for contest_share_val in contest_tally_value['shares']:
                    ...
                    rhs1 = pow(
                        pow(int(contest_share_val['proof']['commitment']['public_key']), 1, p) * pow(
                            valid_public_key_list[i],
                    ...
                    lhs1 = pow(int(contest_tally_value['encrypted_tally']['public_key']),
                               int(contest_share_val['proof']['response']), p)
                ...
                A, B = self.function_modular_product(index, index1, p)
